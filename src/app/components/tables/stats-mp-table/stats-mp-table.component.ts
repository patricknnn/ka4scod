  ...
  getLifetime(): void {
    ...
    this.data.forEach((entry) => {
      this.tableData.push({
        ...
        KD: Math.round(entry.data.lifetime.all.properties.kdRatio * 100) / 100,
        Accuracy: Math.round(entry.data.lifetime.all.properties.accuracy * 100) / 100,
        BestKills: entry.data.lifetime.all.properties.bestKills,
        BestKD: entry.data.lifetime.all.properties.bestKD,
        BestKillstreak: entry.data.lifetime.all.properties.bestKillStreak,
        CurrentWinstreak: entry.data.lifetime.all.properties.currentWinStreak,
      });
    });
    // columns
    this.buildColumns();
