...
export class DynamicFormControlComponent {
    ...
    get errorMessage(): string {
        ...
        message += this.form.controls[this.control.key].hasError('min')
            ? ' must be higher'
            : '';
        message += this.form.controls[this.control.key].hasError('max')
            ? ' must be lower'
            : '';
        message += this.form.controls[this.control.key].hasError('required')
            ? ' is required'
            : '';
        message += this.form.controls[this.control.key].hasError('requiredTrue')
            ? ' is required'
            : '';
        message += this.form.controls[this.control.key].hasError('email')
            ? ' must be email'
            : '';
        message += this.form.controls[this.control.key].hasError('minLength')
            ? ' must be longer'
            : '';
        message += this.form.controls[this.control.key].hasError('maxLength')
            ? ' must be shorter'
            : '';
        message += this.form.controls[this.control.key].hasError('pattern')
            ? ' must match pattern'
            : '';
        return message;
    }
    ...
    updateFormFieldState(): void {
        ...
        let formControl = document.getElementById(
            'form-field-' + this.control.key
        );
        this.isValid
            ? formControl?.classList.remove('mat-form-field-invalid')
            : formControl?.classList.add('mat-form-field-invalid');
    }
